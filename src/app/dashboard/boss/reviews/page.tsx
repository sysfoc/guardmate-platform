...
export default function BossReviewsPage() {
  ...
  const setTab = (tab: 'received' | 'given') => {
    router.push(`/dashboard/boss/reviews?tab=${tab}&page=1`);
  };

  const handlePageChange = (newPage: number) => {
    router.push(`/dashboard/boss/reviews?tab=${activeTab}&page=${newPage}`);
  };
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
        ...
        <div>
          ...
          <p className="text-xs text-[var(--color-text-secondary)] font-medium mt-1">
            See feedback from Mates and reviews you've written.
          </p>
        </div>
          ...
          {loading ? (
            ...
            <Card className="p-10 flex flex-col items-center justify-center text-center border-dashed border-2 border-[var(--color-border-primary)] bg-[var(--color-bg-secondary)]/50">
              ...
              <p className="text-xs text-[var(--color-text-tertiary)] max-w-sm mt-1">
                {activeTab === 'received' 
                  ? "When jobs are completed, Mates will leave feedback here."
                  : "You haven't left any reviews for Mates yet."}
              </p>
            </Card>
          ) : (
            <div className="space-y-4">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <h4 className="text-sm font-bold text-[var(--color-text-primary)]">
                          {activeTab === 'received' ? review.reviewerName : 'Reviewed Mate'}
                        </h4>
