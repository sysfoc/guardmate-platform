  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500 max-w-7xl mx-auto pb-12">
      <div className="px-1">
        ...
        <p className="text-xs sm:text-sm text-[var(--color-text-secondary)] mt-1">Monitor platform revenue, active escrow balances, and transaction history.</p>
      </div>
      ...
      <Card className="p-0 overflow-hidden">
        <div className="p-4 border-b border-[var(--color-border-primary)] bg-[var(--color-bg-secondary)] flex justify-between items-center">
          <h2 className="text-base sm:text-lg font-bold text-[var(--color-text-primary)]">Recent Completed Transactions</h2>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full text-left text-[var(--color-text-secondary)]">
            <thead className="bg-[var(--color-bg-tertiary)] text-[var(--color-text-secondary)] text-[10px] sm:text-xs uppercase font-semibold">
              <tr>
                <th className="px-4 sm:px-6 py-3 whitespace-nowrap">Job / Boss Profile</th>
                <th className="px-4 sm:px-6 py-3 whitespace-nowrap">Date</th>
                <th className="px-4 sm:px-6 py-3 whitespace-nowrap">Method</th>
                <th className="px-4 sm:px-6 py-3 whitespace-nowrap">Job Value</th>
                <th className="px-4 sm:px-6 py-3 whitespace-nowrap text-emerald-600 font-bold">Platform Take</th>
              </tr>
            </thead>
                ...
                transactions.map((tx: any) => (
                  <tr key={tx._id} className="border-b border-[var(--color-border-primary)] hover:bg-[var(--color-bg-tertiary)] transition-colors">
                    <td className="px-4 sm:px-6 py-4">
                      <div className="font-semibold text-[var(--color-text-primary)] truncate max-w-[120px] sm:max-w-none">{tx.jobTitle || 'Unknown Job'}</div>
                      <div className="text-[9px] sm:text-xs opacity-70 truncate max-w-[120px] sm:max-w-none">Job ID: {tx.jobId}</div>
                    </td>
                    <td className="px-4 sm:px-6 py-4 whitespace-nowrap">
                      {new Date(tx.createdAt).toLocaleDateString()}
                    </td>
                    <td className="px-4 sm:px-6 py-4 whitespace-nowrap">
                      <span className={`px-2 py-0.5 sm:px-2.5 sm:py-1 text-[9px] sm:text-xs font-medium rounded-full ${tx.paymentMethod === 'STRIPE' ? 'bg-indigo-100 text-indigo-800' : 'bg-blue-100 text-blue-800'}`}>
                        {tx.paymentMethod}
                      </span>
                    </td>
                    <td className="px-4 sm:px-6 py-4 whitespace-nowrap font-medium text-slate-700">
